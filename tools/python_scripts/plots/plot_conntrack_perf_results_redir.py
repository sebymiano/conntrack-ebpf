...
def create_bpfstats_perf_fig(test, label, narrow=False):
    try:
        with open(f'{sys.path[0]}/../data/perf/results_{test}_v1_redir.perf.yaml', "r") as stream:
            df_v1 = yaml.safe_load(stream)
        with open(f'{sys.path[0]}/../data/perf/results_{test}_v1ns_redir.perf.yaml', "r") as stream:
            df_v1ns = yaml.safe_load(stream)
        with open(f'{sys.path[0]}/../data/perf/results_{test}_v2_redir.perf.yaml', "r") as stream:
