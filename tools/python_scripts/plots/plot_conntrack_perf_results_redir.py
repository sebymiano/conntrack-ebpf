...
def create_bpfstats_perf_fig(test, label, dir, narrow=False):
    try:
        with open(f'{sys.path[0]}/../data/{dir}/perf/results_{test}_v1_redir.perf.yaml', "r") as stream:
            df_v1 = yaml.safe_load(stream)
        with open(f'{sys.path[0]}/../data/{dir}/perf/results_{test}_v1ns_redir.perf.yaml', "r") as stream:
            df_v1ns = yaml.safe_load(stream)
        with open(f'{sys.path[0]}/../data/{dir}/perf/results_{test}_v2_redir.perf.yaml', "r") as stream:
    ...
    output_name = f'{sys.path[0]}/../figures/results_{test}_lat_redir_perf'
...
def create_bpftool_perf_fig(test, metric, label, dir, narrow=False):
    ...
    output_name = f'{sys.path[0]}/../figures/results_{test}_{metric}_redir_perf'
