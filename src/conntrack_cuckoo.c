...
#include "conntrack_cuckoo.skel.h"
...
int main(int argc, const char **argv) {
    struct conntrack_cuckoo_bpf *skel;
    int err;
    ...
    skel = conntrack_cuckoo_bpf__open();
    if (!skel) {
    ...
    err = conntrack_cuckoo_bpf__load(skel);
    if (err) {
...
cleanup:
    cleanup_ifaces();
    conntrack_cuckoo_bpf__destroy(skel);
