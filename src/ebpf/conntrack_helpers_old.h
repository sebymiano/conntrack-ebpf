...
static void conntrack_get_key(struct ct_k *key, const struct packetHeaders *pkt, uint8_t *ipRev,
                              uint8_t *portRev) {
...
static return_action_t handle_tcp_conntrack_forward(struct packetHeaders *pkt,
                                                    struct ct_v *ct_value, uint64_t timestamp) {
...
static return_action_t handle_tcp_conntrack(struct packetHeaders *pkt, struct ct_v *ct_value,
...
static return_action_t handle_tcp_conntrack_reverse(struct packetHeaders *pkt,
                                                    struct ct_v *ct_value, uint64_t timestamp) {
...
static int advance_tcp_state_machine_full(struct packetHeaders *pkt, uint8_t *ipRev,
                                          uint8_t *portRev) {
    struct ct_v *value;
...
static int advance_tcp_state_machine_local(struct ct_k *key, struct packetHeaders *pkt,
                                           uint8_t *ipRev, uint8_t *portRev, struct ct_v *value,
                                           bool *curr_value_set) {
    /* == TCP  == */
    if (pkt->l4proto == IPPROTO_TCP) {
        // If it is a RST, label it as established.
